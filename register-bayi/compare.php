<head><center>
	...
	<a href="/compare/register-bayi/insert.php"style="color:blue; font-size: 30px; ">Back</a><br><br>
...
mysql_select_db ($database, $connection);
		//select colom name
				$query = "SHOW COLUMNS FROM register_bayi_1st";
				//echo $query;
					...
					foreach($colom as $key=>$value)
					    {
					        $table1 = "select ".$value." from register_bayi_1st where motherid = ".$motherid;
							//	$select = "select" for($a=0;$a<=$ii;$a++){ echo $colom[$a].","} "from table1";
							$table2 = "select ".$value." from register_bayi_2nd where motherid = ".$motherid;
							...
							{
								$table3 = "select ".$value." from register_bayi_3rd where motherid = ".$motherid;
							...
							{ 
								$insert2 = "insert into register_bayi_3rd (motherid) values (".$motherid.")";
								$insert = "update register_bayi_3rd set ".$value." = '".$value2."' where motherid = ".$motherid;
