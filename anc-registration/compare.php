<head><center>
	...
	<a href="/compare/anc-registration/insert.php"style="color:blue; font-size: 30px; ">Back</a><br><br>
	<h1>Reconciliation Screen For Bidan Prepopulated Data</h1>
    <h3>Register ANC form</h3>
		<br><br><br><br><br>
</center></head>
<form>
...
mysql_select_db ($database, $connection);
		//select colom name
				$query = "SHOW COLUMNS FROM register_anc_1st";
				//echo $query;
					...
					foreach($colom as $key=>$value)
					    {
					        $table1 = "select ".$value." from register_anc_1st where motherid = ".$motherid;
							//	$select = "select" for($a=0;$a<=$ii;$a++){ echo $colom[$a].","} "from table1";
							$table2 = "select ".$value." from register_anc_2nd where motherid = ".$motherid;
							...
							{
								$table3 = "select ".$value." from register_anc_3rd where motherid = ".$motherid;
							...
							{ 
								$insert2 = "insert into register_anc_3rd (motherid) values (".$motherid.")";
								$insert = "update register_anc_3rd set ".$value." = '".$value2."' where motherid = ".$motherid;
