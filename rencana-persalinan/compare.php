<head><center>
	...
	<a href="/compare/rencana-persalinan/insert.php"style="color:blue; font-size: 30px; ">Back</a><br><br>
	<h1>Reconciliation Screen For Bidan Prepopulated Data</h1>
    <h3>Rencana Persalinan form</h3>
		<br><br><br><br><br>
</center></head>
<form>
...
mysql_select_db ($database, $connection);
		//select colom name
				$query = "SHOW COLUMNS FROM rencana_persalinan_1st";
				//echo $query;
					...
					foreach($colom as $key=>$value)
					    {
					        $table1 = "select ".$value." from rencana_persalinan_1st where motherid = ".$motherid;
							//	$select = "select" for($a=0;$a<=$ii;$a++){ echo $colom[$a].","} "from table1";
							$table2 = "select ".$value." from rencana_persalinan_2nd where motherid = ".$motherid;
							...
							{
								$table3 = "select ".$value." from rencana_persalinan_3rd where motherid = ".$motherid;
							...
							{ 
								$insert2 = "insert into rencana_persalinan_3rd (motherid) values (".$motherid.")";
								$insert = "update rencana_persalinan_3rd set ".$value." = '".$value2."' where motherid = ".$motherid;
