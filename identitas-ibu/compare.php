<head><center>
	...
	<a href="/compare/identitas-ibu/insert.php"style="color:blue; font-size: 30px; ">Back</a><br><br>
...
mysql_select_db ($database, $connection);
		//select colom name
				$query = "SHOW COLUMNS FROM identitas_ibu_1st";
				//echo $query;
					...
					foreach($colom as $key=>$value)
					    {
					        $table1 = "select ".$value." from identitas_ibu_1st where motherid = ".$motherid;
							//	$select = "select" for($a=0;$a<=$ii;$a++){ echo $colom[$a].","} "from table1";
							$table2 = "select ".$value." from identitas_ibu_2nd where motherid = ".$motherid;
							...
							{
								$table3 = "select ".$value." from identitas_ibu_3rd where motherid = ".$motherid;
							...
							{ 
								$insert2 = "insert into identitas_ibu_3rd (motherid) values (".$motherid.")";
								$insert = "update identitas_ibu_3rd set ".$value." = '".$value2."' where motherid = ".$motherid;
